...
VEML6030::VEML6030(uint8_t address){  _address = address; } //Constructor for I2C

bool VEML6030::begin( TwoWire &wirePort )
{
...
void VEML6030::setGain(float gainVal){
...
void VEML6030::setIntegTime(uint16_t time){ 
...
// sensor) of the ambient light sensor. Higher integration time leads to better
// resolution but slower sensor refresh times. 
uint16_t VEML6030::readIntegTime(){
...
void VEML6030::setProtect(uint8_t protVal){
...
uint8_t VEML6030::readIntSetting(){
...
uint8_t VEML6030::readPowSavEnabled(){
...
void VEML6030::setPowSavMode(uint16_t modeVal){
...
// increases power saving. The datasheet suggests enabling these modes when
// continually sampling the sensor. 
uint8_t VEML6030::readPowSavMode(){
...
uint8_t VEML6030::readInterrupt(){
...
void VEML6030::setIntLowThresh(uint32_t luxVal){
...
uint32_t VEML6030::readLowThresh(){
...
// It takes a lux value as its paramater.
void VEML6030::setIntHighThresh(uint32_t luxVal){
...
uint32_t VEML6030::readHighThresh(){
...
// determined based on current gain and integration time settings. If the lux
// value exceeds 1000 then a compensation formula is applied to it. 
uint32_t VEML6030::readWhiteLight(){
...
// etc. etc. 
uint32_t VEML6030::_luxCompensation(uint32_t _luxVal){ 
...
uint32_t VEML6030::_calculateLux(uint16_t _lightBits){
...
// that.  
uint16_t VEML6030::_calculateBits(uint32_t _luxVal){
...
// position.
void VEML6030::_writeRegister(uint8_t _wReg, uint16_t _mask,\
...
uint16_t VEML6030::_readRegister(uint8_t _reg)
{
